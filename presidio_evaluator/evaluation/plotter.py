...
if __name__ == "__main__":
    ...
    model_errors.extend([ModelError(ErrorType.WrongEntity, "X", "Z", "i", "")] * 5)
    model_errors.extend([ModelError(ErrorType.WrongEntity, "Y", "X", "g", "")] * 6)
    model_errors.extend([ModelError(ErrorType.WrongEntity, "Y", "Z", "i", "")] * 6)
    model_errors.extend([ModelError(ErrorType.WrongEntity, "Z", "X", "g", "")] * 7)
    ...
    class MockTokensModel(BaseModel):
        ...
        def __init__(
            ...
            **kwargs,
            ...
            super().__init__(
                entities_to_keep=entities_to_keep, verbose=verbose, **kwargs
            )
            self.prediction = prediction

        def predict(self, sample, **kwargs) -> List[str]:
            return self.prediction

        def batch_predict(self, dataset, **kwargs) -> List[List[str]]:
    ...
    final_result.model_errors = model_errors
    entities, confmatrix = final_result.to_confusion_matrix()

    plotter = Plotter(results=final_result, save_as=None)
    plotter.plot_scores()
    plotter.plot_most_common_tokens()
    plotter.plot_confusion_matrix(entities, confmatrix)
