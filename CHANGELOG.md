...
- **Gold spans are reconstructed from exact span identity instead of label runs** — `span_to_tag` can now return, alongside the per-token labels, the index of the span that produced each label (`return_span_ids=True`); the ids survive overlap resolution, so a span split by a higher-score overlap keeps one identity. `InputSample` stores them as `span_ids` when tags are created from spans, and `BaseModel.predict_dataset` carries them into a new `annotation_span_id` column (always present; all None for samples whose tags were provided directly). `SpanEvaluator` prefers these ids over the merge-key columns when reconstructing annotation spans, so gold boundaries are exact rather than inferred: two adjacent entities of the *same* type (`"She visited Paris , London"`) now stay two spans at every level, which merge keys alone could not distinguish. Prediction spans keep the existing reconstruction, and DataFrames without id values fall back to the merge keys, then to the visible label.
