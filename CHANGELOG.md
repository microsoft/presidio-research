...
- **Gold spans are reconstructed from exact span identity instead of label runs** — `BaseModel.predict_dataset` now attaches an `annotation_span_id` column: the index of the gold span covering each token, taken from `InputSample.spans` before they are flattened to per-token labels. `SpanEvaluator` prefers these ids over the merge-key columns when reconstructing annotation spans, so gold boundaries are exact rather than inferred: two adjacent entities of the *same* type (`"She visited Paris , London"`) now stay two spans at every level, which merge keys alone could not distinguish. Prediction spans keep the existing reconstruction, and DataFrames without the column (built by hand, or from samples that carry no spans) keep their previous behaviour.
